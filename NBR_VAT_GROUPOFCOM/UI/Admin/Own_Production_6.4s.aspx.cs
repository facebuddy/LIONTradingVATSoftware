...
{
    public partial class Own_Production_6__4s : Page, IRequiresSessionState
    
    {
            ...
            try
            {
                if (this.validation())
                {
                    ...
                    if (contructualProductionIssueMasterDAO != null)
                    {
                        ...
                        if (!contructualProductionBLL.insertCPIssueDataOwnProdction(contructualProductionIssueMasterDAO, arrayLists))
                        {
        ...
        protected void btnShowIngredience_OnClick(object sender, EventArgs e)
        {
            ...
            {
                BLL.Utility.InsertErrorTrack(exception.Message, "Own_Production_6.4", "btnShowIngredience_OnClick");
            }
        }
        ...
        private ContructualProductionIssueMasterDAO fillCPMasterData(ContructualProductionIssueMasterDAO objCPMaster)
        {
            ...
            {
                BLL.Utility.InsertErrorTrack(exception.Message, "Own_Production_6.4", "fillCPMasterData");
            }
            return objCPMaster;
        }
        ...
        private decimal getAvailableStock(short itemId)
        {
            ...
            SaleBLL saleBLL = new SaleBLL();
            DateTime dateTime = DateTime.ParseExact(this.txtChallanDate.Text.Trim(), "dd/MM/yyyy", CultureInfo.InvariantCulture);
            DataTable availStockforTransferProduct = saleBLL.getAvailStockforTransferProduct(itemId, dateTime);
            if (availStockforTransferProduct.Rows != null && availStockforTransferProduct.Rows[0]["item_type"].ToString() == "I")
            {
                num = Convert.ToDecimal(availStockforTransferProduct.Rows[0]["availStock"]);
            }
            return num;
        }
        ...
        private void getChallanNo()
        {
            ...
            DataTable creditNoteNo = craDebBLL.GetCreditNoteNo(10, num, num1);
        ...
        private void getCPDetailData()
        {
            ...
            ContructualProductionIssueDAO contructualProductionIssueDAO = new ContructualProductionIssueDAO()
            {
                ...
                CategoryName = this.drpCategory.SelectedItem.ToString(),
                SubCategoryName = this.drpSubCategory.SelectedItem.ToString(),
        ...
        private void getUsedQuantity()
        {
            ...
            try
            {
                ...
                DataTable quantityToMadePerProduct = contructualProductionBLL.GetQuantityToMadePerProduct(num1, (this.drpPriceDeclarationNo.SelectedValue != "" ? Convert.ToInt32(this.drpPriceDeclarationNo.SelectedItem.Value) : 0));
                if (quantityToMadePerProduct.Rows.Count > 0)
                {
                    for (int i = 0; i < quantityToMadePerProduct.Rows.Count; i++)
                    {
                        ...
                        num6 = Convert.ToDecimal(Utilities.RoundUpToWithString(Convert.ToDecimal(quantityToMadePerProduct.Rows[i]["quantity"]), 2).ToString());
        ...
        private void loadPertyInfo()
        {
            ...
            if (dataTable != null && dataTable.Rows.Count > 0)
            {
                ...
                this.txtAddress.Text = dataTable.Rows[0]["party_address"].ToString();
            }
        ...
        private bool validation()
        {
            ...
            {
                today = DateTime.ParseExact(this.txtMfgDate.Text.Trim(), "dd/MM/yyyy", CultureInfo.InvariantCulture);
            }
            if (this.txtExpDate.Text != "")
            {
                dateTime = DateTime.ParseExact(this.txtExpDate.Text.Trim(), "dd/MM/yyyy", CultureInfo.InvariantCulture);
            }
            if (today > dateTime)
            {
