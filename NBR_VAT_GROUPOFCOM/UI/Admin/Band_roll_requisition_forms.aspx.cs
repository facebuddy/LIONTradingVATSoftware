...
{
    public partial class Band_roll_requisition_forms : Page, IRequiresSessionState
    {
      

        public Band_roll_requisition_forms()
        {
        }
            ...
            try
            {
                if (this.Mastervalidation())
                {
                    ...
                    if (bandrollIManagementMasterDAO != null)
                    {
                        ...
                        else if (!bandrollManagementBLL.insertBandrollIssueData(bandrollIManagementMasterDAO, item))
                        {
        ...
        private void LoadFinishProduct()
        {
            ...
            try
            {
                DataTable finishGoodsProductionBiriCigarate = addItemBLL.GetFinishGoodsProductionBiriCigarate();
        ...
        private bool validation()
        {
            ...
            if (!this.Session["hs_code"].ToString().EndsWith("20.00") || !(this.drpPacketType.SelectedValue == "-99"))
            {
                return true;
            }
            this.msgBox.AddMessage("Select Packet Type", MsgBoxs.enmMessageType.Attention);
            return false;
        }
    }
}