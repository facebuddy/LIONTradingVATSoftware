...
public class Logins1 : Page, IRequiresSessionState
{
    ...
    static Logins1()
    {
        Logins1.URL = WebConfigurationManager.AppSettings["apiUrl"];
        Logins1.version = WebConfigurationManager.AppSettings["apiVersion"];
    }
    ...
    protected void btnLogin_Click(object sender, EventArgs e)
    {
        ...
        if (this.Validation())
        {
            ...
            if (Crypto.VerifyHashedPassword(str4, str5))
            {
                ...
                {
                    item = (masterOrgInfo[1] != "S" ? "Bss_unts.aspx" : "Admin_Dashboard.aspx");
                }
                else
                {
                    item = "OrgSetup.aspx";
    ...
    private static int callService()
    {
        ...
        try
        {
            HttpWebRequest httpWebRequest = (HttpWebRequest)WebRequest.Create(string.Concat(Logins1.URL, "extenddemoperiod"));
            ...
            httpWebRequest.Headers.Set("version", Logins1.version);
            ...
            Logins1.ResponseResult responseResult = (new JavaScriptSerializer()).Deserialize<Logins1.ResponseResult>(end);
    ...
    private static Logins1.LicenseUpdateResponse callServiceForUpdateServicePack(string publicKey)
    {
        Logins1.LicenseUpdateResponse licenseUpdateResponse;
        Logins1.LicenseUpdateResponse licenseUpdateResponse1 = new Logins1.LicenseUpdateResponse();
        try
        {
            HttpWebRequest httpWebRequest = (HttpWebRequest)WebRequest.Create(string.Concat(Logins1.URL, "updateservicepack"));
            string macId = UtilityK.getMacId();
            string hostName = Dns.GetHostName();
            string str = Dns.GetHostByName(hostName).AddressList[0].ToString();
            ...
            HttpWebResponse response = (HttpWebResponse)httpWebRequest.GetResponse();
            string end = (new StreamReader(response.GetResponseStream())).ReadToEnd();
            licenseUpdateResponse1 = (new JavaScriptSerializer()).Deserialize<Logins1.LicenseUpdateResponse>(end);
