...
public class Bss_unts : Page, IRequiresSessionState
{
    protected DropDownList drpSchema;

    protected DropDownList drpBusinessUnitBranch;

    protected Button btnBusinessUnit;
  ...
  //  protected MsgBoxs msgBox;
    ...
    static Bss_unts()
    {
        Bss_unts.URL = WebConfigurationManager.AppSettings["apiUrl"];
        Bss_unts.version = WebConfigurationManager.AppSettings["apiVersion"];
    }
        ...
        try
        {
            ...
            if (this.Validations())
            {
                ...
                if (this.uBll.getUserIDBool(this.Session["suserId"].ToString(), this.Session["encPassword"].ToString()) && businessInformation != null && businessInformation.Rows.Count > 0)
                {
                    ...
                    base.Response.Redirect("Admin_Dashboards.aspx", false);
                }
            }
        }
    ...
    private void GetBusinessUnit()
    {
        ...
        catch (Exception exception1)
        {
            ...
            Utility.InsertErrorTrackNew(exception.Message, "Bss_unit.aspx", "GetBusinessUnit", fileLineNumber);
        }
    }
