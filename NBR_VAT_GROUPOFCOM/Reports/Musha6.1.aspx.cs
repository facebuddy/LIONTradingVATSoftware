...
{
    public partial class Musha6__1 : Page, IRequiresSessionState
    {
        ...
        private void GenerateHeader(HSSFWorkbook workbook, ISheet sheet, HSSFCellStyle headerStyle)
        {
            ...
            this.excelUtility.CreateCell(row, 0, "পণ্য/ সেবার উপকরণ ক্রয়", headerStyle);
            this.excelUtility.CreateCell(row1, 0, "ক্রমিক সংখ্যা", headerStyle);
            this.excelUtility.CreateCell(row1, 1, "তারিখ", headerStyle);
            this.excelUtility.CreateCell(row1, 2, "মজুদ উপকরণের প্রারম্ভিক জের", headerStyle);
            this.excelUtility.CreateCell(row1, 4, "ক্রয়কৃত উপকরণ", headerStyle);
            this.excelUtility.CreateCell(row1, 18, "উপকরণের প্রান্তিক জের", headerStyle);
            ...
            this.excelUtility.CreateCell(row2, 4, "চালানপত্র / বিল অব এন্ট্রি নম্বর", headerStyle);
            this.excelUtility.CreateCell(row2, 5, "তারিখ", headerStyle);
            this.excelUtility.CreateCell(row2, 6, "বিক্রেতা/সরবরাহকারী", headerStyle);
            this.excelUtility.CreateCell(row2, 9, "বিবরণ", headerStyle);
            this.excelUtility.CreateCell(row2, 10, "পরিমাণ", headerStyle);
            this.excelUtility.CreateCell(row2, 11, "মূল্য (সকল প্রকার কর ব্যতীত)", headerStyle);
            this.excelUtility.CreateCell(row2, 12, "সম্পূরক শুল্ক (যদি থাকে)", headerStyle);
            this.excelUtility.CreateCell(row2, 13, "মূসক", headerStyle);
            this.excelUtility.CreateCell(row2, 14, "মোট উপকরণের পরিমাণ", headerStyle);
            this.excelUtility.CreateCell(row2, 16, "পণ্য প্রস্তুতকরণ / উৎপাদনে ব্যবহার", headerStyle);
            this.excelUtility.CreateCell(row2, 18, "পরিমাণ", headerStyle);
            this.excelUtility.CreateCell(row2, 19, "মূল্য (সকল প্রকার কর ব্যতীত)", headerStyle);
            this.excelUtility.CreateCell(row3, 6, "নাম", headerStyle);
            this.excelUtility.CreateCell(row3, 7, "ঠিকানা", headerStyle);
            this.excelUtility.CreateCell(row3, 8, "নিবন্ধন /তালিকাভুক্তি/ জাতীয় পরিচয়পত্র নং", headerStyle);
            this.excelUtility.CreateCell(row3, 14, "পরিমাণ", headerStyle);
            this.excelUtility.CreateCell(row3, 15, "মূল্য", headerStyle);
            this.excelUtility.CreateCell(row3, 16, "পরিমাণ (একক)", headerStyle);
            this.excelUtility.CreateCell(row3, 17, "মূল্য (সকল প্রকার কর ব্যতীত)", headerStyle);
            for (int i = 0; i <= 20; i++)
            {
                int num = i + 1;
                this.excelUtility.CreateCell(row4, i, string.Concat("(", num, ")"), headerStyle);
            }
        ...
        private void GetDataExcel(string displayHtml, DataTable dt, int i, string SEarly_quantity, string unitCodeOp, decimal early_Price, int precision, string currentDate, string pQuantity, string unitAll, string pPrice, string pSD, string pVAT, string Ssurplus_quantity, decimal surplus_price, string Ssurplus_Proquantity, string proQuantity, decimal totalProPrice, decimal surplus_proprice, ISheet sheet, HSSFCellStyle headerStyle, int rowIndex)
        {
            ...
            excelUtility.CreateCell(row1, 1, dateTime.ToString("dd/MM/yyyy") ?? "", headerStyle);
            if (SEarly_quantity != "0")
            {
                ...
                excelUtility1.CreateCell(row, 2, string.Concat(sEarlyQuantity), headerStyle);
            }
            else
            {
                this.excelUtility.CreateCell(row, 2, "--", headerStyle);
            }
            this.excelUtility.CreateCell(row, 3, Utilities.RoundUpToWithString(early_Price, precision) ?? "", headerStyle);
            this.excelUtility.CreateCell(row, 4, string.Concat(dt.Rows[i]["challan_no"]), headerStyle);
            this.excelUtility.CreateCell(row, 5, currentDate ?? "", headerStyle);
            ...
            excelUtility2.CreateCell(row2, 6, string.Concat(item), headerStyle);
            ...
            excelUtility3.CreateCell(row3, 7, string.Concat(obj), headerStyle);
            ...
            excelUtility4.CreateCell(row4, 8, string.Concat(item1), headerStyle);
            this.excelUtility.CreateCell(row, 9, string.Concat(dt.Rows[i]["item_name"]), headerStyle);
            if (pQuantity != "0")
            {
                ExcelUtility excelUtility5 = this.excelUtility;
                object[] objArray = new object[] { pQuantity, '(', unitAll, ')' };
                excelUtility5.CreateCell(row, 10, string.Concat(objArray), headerStyle);
            }
            else
            {
                displayHtml = string.Concat(displayHtml, "<td style='text-align: right;'>-- </td>");
                this.excelUtility.CreateCell(row, 10, "--", headerStyle);
            }
            this.excelUtility.CreateCell(row, 11, pPrice ?? "", headerStyle);
            this.excelUtility.CreateCell(row, 12, pSD ?? "", headerStyle);
            this.excelUtility.CreateCell(row, 13, pVAT ?? "", headerStyle);
            ExcelUtility excelUtility6 = this.excelUtility;
            object[] ssurplusQuantity = new object[] { Ssurplus_quantity, '(', unitAll, ')' };
            excelUtility6.CreateCell(row, 14, string.Concat(ssurplusQuantity), headerStyle);
            this.excelUtility.CreateCell(row, 15, Utilities.RoundUpToWithString(surplus_price, precision) ?? "", headerStyle);
            ExcelUtility excelUtility7 = this.excelUtility;
            object[] objArray1 = new object[] { proQuantity, '(', unitAll, ')' };
            excelUtility7.CreateCell(row, 16, string.Concat(objArray1), headerStyle);
            this.excelUtility.CreateCell(row, 17, Utilities.RoundUpToWithString(totalProPrice, precision) ?? "", headerStyle);
            ExcelUtility excelUtility8 = this.excelUtility;
            object[] ssurplusProquantity = new object[] { Ssurplus_Proquantity, '(', unitAll, ')' };
            excelUtility8.CreateCell(row, 18, string.Concat(ssurplusProquantity), headerStyle);
            this.excelUtility.CreateCell(row, 19, Utilities.RoundUpToWithString(surplus_proprice, precision) ?? "", headerStyle);
        ...
        private void GetDataExcelIfIZ(string displayHtml, int i, string opbld, string SEarly_quantity, string unitCodeOp, decimal early_Price, int precision, string unitAll, ISheet sheet, HSSFCellStyle headerStyle)
        {
            ...
            this.excelUtility.CreateCell(row, 1, opbld, headerStyle);
            ExcelUtility excelUtility = this.excelUtility;
            object[] sEarlyQuantity = new object[] { SEarly_quantity, " ", '(', unitCodeOp, ')' };
            excelUtility.CreateCell(row, 2, string.Concat(sEarlyQuantity), headerStyle);
            this.excelUtility.CreateCell(row, 3, Utilities.RoundUpToWithString(early_Price, precision) ?? "", headerStyle);
            ...
            excelUtility1.CreateCell(row, 18, string.Concat(objArray), headerStyle);
            this.excelUtility.CreateCell(row, 19, Utilities.RoundUpToWithString(early_Price, precision) ?? "", headerStyle);
        ...
        private void GetTotalExcel(string displayHtml, int rowIndex, decimal TotalPurchasePrice, decimal TotalPurchaseSD, decimal TotalPurchaseVAT, int precision, ISheet sheet, HSSFCellStyle headerStyle, HSSFWorkbook workbook)
        {
            ...
            this.excelUtility.CreateCell(row, 11, Utilities.RoundUpToWithString(TotalPurchasePrice, precision) ?? "", headerStyle);
            this.excelUtility.CreateCell(row, 12, Utilities.RoundUpToWithString(TotalPurchaseSD, precision) ?? "", headerStyle);
            this.excelUtility.CreateCell(row, 13, Utilities.RoundUpToWithString(TotalPurchaseVAT, precision) ?? "", headerStyle);
