...
{
    public partial class Mushak6__2 : Page, IRequiresSessionState
    {
        ...
        private void GenerateHeader(HSSFWorkbook workbook, ISheet sheet, HSSFCellStyle headerStyle)
        {
            ...
            IRow row3 = sheet.CreateRow(5);
            this.excelUtility.CreateCell(row, 0, "পণ্য/ সেবার বিক্রয়", headerStyle);
            this.excelUtility.CreateCell(row1, 0, "ক্রমিক সংখ্যা", headerStyle);
            this.excelUtility.CreateCell(row1, 1, "তারিখ", headerStyle);
            this.excelUtility.CreateCell(row1, 2, "উৎপাদিত পণ্যের প্রারম্ভিক জের", headerStyle);
            this.excelUtility.CreateCell(row1, 4, "উৎপাদন", headerStyle);
            this.excelUtility.CreateCell(row1, 6, "মোট উৎপাদিত পণ্য/সেবা", headerStyle);
            this.excelUtility.CreateCell(row1, 8, "ক্রেতা/ সরবরাহগ্রহীতা", headerStyle);
            this.excelUtility.CreateCell(row1, 11, "চালানপত্রের বিবরণ", headerStyle);
            this.excelUtility.CreateCell(row1, 13, "বিক্রিত/সরবরাহকৃত পণ্যের বিবরণ", headerStyle);
            this.excelUtility.CreateCell(row1, 18, "পণ্যের প্রান্তিক জের", headerStyle);
            this.excelUtility.CreateCell(row1, 20, "মন্তব্য", headerStyle);
            this.excelUtility.CreateCell(row2, 2, "পরিমাণ (একক)", headerStyle);
            this.excelUtility.CreateCell(row2, 3, "মূল্য (সকল প্রকার কর ব্যতীত)", headerStyle);
            this.excelUtility.CreateCell(row2, 4, "পরিমাণ (একক)", headerStyle);
            this.excelUtility.CreateCell(row2, 5, "মূল্য (সকল প্রকার কর ব্যতিত)", headerStyle);
            this.excelUtility.CreateCell(row2, 6, "পরিমাণ (একক)", headerStyle);
            this.excelUtility.CreateCell(row2, 7, "মূল্য (সকল প্রকার কর ব্যতীত)", headerStyle);
            this.excelUtility.CreateCell(row2, 8, "নাম", headerStyle);
            this.excelUtility.CreateCell(row2, 9, "ঠিকানা", headerStyle);
            this.excelUtility.CreateCell(row2, 10, "নিবন্ধন /তালিকাভুক্তি/ জাতীয় পরিচয়পত্র নং", headerStyle);
            this.excelUtility.CreateCell(row2, 11, "নম্বর", headerStyle);
            this.excelUtility.CreateCell(row2, 12, "তারিখ", headerStyle);
            this.excelUtility.CreateCell(row2, 13, "বিবরণ", headerStyle);
            this.excelUtility.CreateCell(row2, 14, "পরিমাণ", headerStyle);
            this.excelUtility.CreateCell(row2, 15, "করযোগ্য মূল্য", headerStyle);
            this.excelUtility.CreateCell(row2, 16, "সম্পূরক শুল্ক(যদি থাকে)", headerStyle);
            this.excelUtility.CreateCell(row2, 17, "মুসক", headerStyle);
            this.excelUtility.CreateCell(row2, 18, "পরিমাণ (একক)", headerStyle);
            this.excelUtility.CreateCell(row2, 19, "মূল্য (সকল প্রকার কর ব্যতিত)", headerStyle);
            for (int i = 0; i <= 20; i++)
            {
                int num = i + 1;
                this.excelUtility.CreateCell(row3, i, string.Concat("(", num, ")"), headerStyle);
            }
        ...
        private void GetDataExcelIfIGZ(string displayHtml, DataTable dt, int i, string currentDate, decimal prarombikJerQty, decimal prarombikJerValue, decimal column5, decimal column6, decimal result, decimal result8, decimal sQuantity, decimal sPrice, decimal sSD, decimal sVAT, decimal prantikJerQty, decimal prantikJerValue, int precision, string productionUnitCode, ISheet sheet, HSSFCellStyle headerStyle, HSSFWorkbook workbook, int rowIndex)
        {
            ...
            this.excelUtility.CreateCell(row, 0, string.Concat(i + 1), headerStyle);
            ...
            this.excelUtility.CreateCell(row, 1, str1, headerStyle);
            ExcelUtility excelUtility = this.excelUtility;
            object[] objArray = new object[] { prarombikJerQty, "(", str, ")" };
            excelUtility.CreateCell(row, 2, string.Concat(objArray), headerStyle);
            this.excelUtility.CreateCell(row, 3, Utilities.RoundUpToWithString(prarombikJerValue, precision), headerStyle);
            this.excelUtility.CreateCell(row, 4, string.Concat(Utilities.formatFraction(column5), "(", str, ")"), headerStyle);
            this.excelUtility.CreateCell(row, 5, Utilities.RoundUpToWithString(column6, precision), headerStyle);
            this.excelUtility.CreateCell(row, 6, string.Concat(Utilities.formatFraction(result), " (", str, ")"), headerStyle);
            this.excelUtility.CreateCell(row, 7, Utilities.RoundUpToWithString(result8, precision), headerStyle);
            this.excelUtility.CreateCell(row, 8, string.Concat(dt.Rows[i]["party_name"]), headerStyle);
            this.excelUtility.CreateCell(row, 9, string.Concat(dt.Rows[i]["party_address"]), headerStyle);
            if (dt.Rows[i]["reg_type"].ToString() != "5")
            {
                this.excelUtility.CreateCell(row, 10, string.Concat(dt.Rows[i]["party_bin"]), headerStyle);
            }
            else
            {
                this.excelUtility.CreateCell(row, 10, string.Concat(dt.Rows[i]["national_id_no"]), headerStyle);
            }
            this.excelUtility.CreateCell(row, 11, string.Concat(dt.Rows[i]["challan_no"]), headerStyle);
            this.excelUtility.CreateCell(row, 12, currentDate ?? "", headerStyle);
            this.excelUtility.CreateCell(row, 13, string.Concat(dt.Rows[i]["item_name"]), headerStyle);
            this.excelUtility.CreateCell(row, 14, string.Concat(Utilities.formatFraction(sQuantity), "(", str, ")"), headerStyle);
            this.excelUtility.CreateCell(row, 15, Utilities.RoundUpToWithString(sPrice, precision) ?? "", headerStyle);
            this.excelUtility.CreateCell(row, 16, Utilities.RoundUpToWithString(sSD, precision) ?? "", headerStyle);
            this.excelUtility.CreateCell(row, 17, Utilities.RoundUpToWithString(sVAT, precision) ?? "", headerStyle);
            this.excelUtility.CreateCell(row, 18, string.Concat(Utilities.formatFraction(prantikJerQty), "(", str, ")"), headerStyle);
            this.excelUtility.CreateCell(row, 19, Utilities.RoundUpToWithString(prantikJerValue, precision) ?? "", headerStyle);
            this.excelUtility.CreateCell(row, 20, string.Concat(dt.Rows[i]["remarks"]), headerStyle);
        }
        ...
        {
            IRow row = sheet.CreateRow(rowIndex);
            this.excelUtility.CreateCell(row, 0, string.Concat(i), headerStyle);
            this.excelUtility.CreateCell(row, 1, currentDate ?? "", headerStyle);
            ExcelUtility excelUtility = this.excelUtility;
            ...
            {
                this.excelUtility.CreateCell(row, num, "--", headerStyle);
            }
            ExcelUtility excelUtility1 = this.excelUtility;
            object[] objArray1 = new object[] { prarombikJerQty, '(', productionUnitCode, ')' };
            excelUtility1.CreateCell(row, 18, string.Concat(objArray1), headerStyle);
            this.excelUtility.CreateCell(row, 19, Utilities.RoundUpToWithString(prarombikJerValue, precision) ?? "", headerStyle);
            this.excelUtility.CreateCell(row, 20, "--", headerStyle);
        }
        ...
        {
            IRow row = sheet.CreateRow(rowIndex);
            this.excelUtility.CreateCell(row, 0, "মোট", headerStyle);
            this.excelUtility.CreateCell(row, 4, Utilities.RoundUpToWithString(pTotalQuantity, precision) ?? "", headerStyle);
            this.excelUtility.CreateCell(row, 5, Utilities.RoundUpToWithString(pTotalPrice, precision) ?? "", headerStyle);
            this.excelUtility.CreateCell(row, 6, Utilities.RoundUpToWithString(ptTotalQuantity, precision) ?? "", headerStyle);
            this.excelUtility.CreateCell(row, 7, Utilities.RoundUpToWithString(ptTotalPrice, precision) ?? "", headerStyle);
            this.excelUtility.CreateCell(row, 14, Utilities.RoundUpToWithString(slTotalQuantity, precision) ?? "", headerStyle);
            this.excelUtility.CreateCell(row, 15, Utilities.RoundUpToWithString(slTotalPrice, precision) ?? "", headerStyle);
            this.excelUtility.CreateCell(row, 16, Utilities.RoundUpToWithString(sTotalSD, precision) ?? "", headerStyle);
            this.excelUtility.CreateCell(row, 17, Utilities.RoundUpToWithString(sTotalVat, precision) ?? "", headerStyle);
            this.excelUtility.CreateCell(row, 18, Utilities.RoundUpToWithString(sTotalQuantity, precision) ?? "", headerStyle);
            this.excelUtility.CreateCell(row, 19, Utilities.RoundUpToWithString(sTotalPrice, precision) ?? "", headerStyle);
            this.excelUtility.CreateCell(row, 20, "", headerStyle);
