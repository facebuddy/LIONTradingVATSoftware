...
{
    public partial class SubForm_Ks : Page, IRequiresSessionState
    {
            ...
            try
            {
                ...
                if (salesList != null)
                {
                    foreach (var item in salesList)
                    {
                        ...
                        str = string.Concat(str, "<td>", item.prduct_description, "</td>");
        ...
        {
            string str1 =Convert.ToDateTime(HttpContext.Current.Session["fDateSubs"]).ToString("yyyy-MM-dd");
            string str2 = Convert.ToDateTime(HttpContext.Current.Session["tDateSubs"]).ToString("yyyy-MM-dd");

            //string startDate = Request.QueryString["start_date"];

            //string endDate = Request.QueryString["end_date"];
            LoadReport();
          //  LoadMukhak(str1, str2);
        }
    }
}