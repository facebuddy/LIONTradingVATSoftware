...
{
    public partial class SubForm_k20s : Page, IRequiresSessionState
    {
        ...
        private void LoadReport()
        {
            try
            {
                ...
                DataTable vatReturnPart4DataNotregisteredSubReport = this.dbBLL.getVatReturnPart4DataNotregisteredSubReport(str1, str2, num7);
                if (vatReturnPart4DataNotregisteredSubReport != null && vatReturnPart4DataNotregisteredSubReport.Rows.Count > 0)
                {
                    for (int i = 0; i < vatReturnPart4DataNotregisteredSubReport.Rows.Count; i++)
                    {
                        num = Convert.ToDecimal(vatReturnPart4DataNotregisteredSubReport.Rows[i]["price"]);
                        num1 = Convert.ToDecimal(vatReturnPart4DataNotregisteredSubReport.Rows[i]["sd"]);
                        num2 = Convert.ToDecimal(vatReturnPart4DataNotregisteredSubReport.Rows[i]["vat"]);
                        ...
                        str = string.Concat(str, "<td>", vatReturnPart4DataNotregisteredSubReport.Rows[i]["details"].ToString(), "</td>");
                        str = string.Concat(str, "<td>", vatReturnPart4DataNotregisteredSubReport.Rows[i]["hs_code"].ToString(), "</td>");
                        str = string.Concat(str, "<td>", vatReturnPart4DataNotregisteredSubReport.Rows[i]["item_name"].ToString(), "</td>");
                        ...
                        str = string.Concat(str, "<td>", vatReturnPart4DataNotregisteredSubReport.Rows[i]["remarks"].ToString(), "</td>");
