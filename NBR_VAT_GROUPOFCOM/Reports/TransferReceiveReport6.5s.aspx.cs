...
{
    public partial class TransferReceiveReport6__5s : Page, IRequiresSessionState
    {
        ...
        public TransferReceiveReport6__5s()
        {
        }
        ...
        private void fillChallanNo()
        {
            ...
            try
            {
                ...
                DataTable rcvChallanNoforRepoort = productTransferBLL.getRcvChallanNoforRepoort(num1, num);
                this.drpChallanNo.DataSource = rcvChallanNoforRepoort;
                this.drpChallanNo.DataTextField = rcvChallanNoforRepoort.Columns["challan_no"].ToString();
                this.drpChallanNo.DataValueField = rcvChallanNoforRepoort.Columns["transfer_id"].ToString();
        ...
        private void GetBranchRcvrInformation()
        {
            ...
            DataTable branchInformationFormanager = challanBLL.GetBranchInformationFormanager(num);
            if (branchInformationFormanager != null && branchInformationFormanager.Rows.Count > 0)
            {
                this.drpReceipentBranch.Items.Clear();
                this.drpReceipentBranch.DataSource = branchInformationFormanager;
                this.drpReceipentBranch.DataTextField = branchInformationFormanager.Columns["branch_unit_name"].ToString();
                this.drpReceipentBranch.DataValueField = branchInformationFormanager.Columns["org_branch_reg_id"].ToString();
        ...
        private void showReport()
        {
            try
            {
                ...
                if (this.btnShowReport.Text == "Show Report")
                {
                    ...
                    DataTable transferRcvInfo = this.objBLL.getTransferRcvInfo(num1, num2, num);
                    if (transferRcvInfo.Rows.Count <= 0)
                    {
                    ...
                    else
                    {
                        ...
                        this.lblChallanNo.Text = transferRcvInfo.Rows[0]["challan_no"].ToString();
                        this.lblIssueDate.Text = transferRcvInfo.Rows[0]["issues_date"].ToString();
                        this.lblIssueTime.Text = transferRcvInfo.Rows[0]["issues_time"].ToString();
                        this.txtTransport.Text = transferRcvInfo.Rows[0]["vehicle_no"].ToString();
                        string str = "";
                        for (int i = 0; i < transferRcvInfo.Rows.Count; i++)
                        {
                            ...
                            str = string.Concat(str, "<td style='border:1px solid gray'>", transferRcvInfo.Rows[i]["item_name"].ToString(), "</td>");
                            string str3 = str;
                            string[] str4 = new string[] { str3, "<td style='border:1px solid gray;text-align: center'>", transferRcvInfo.Rows[i]["quantity"].ToString(), " ", transferRcvInfo.Rows[i]["unit_code"].ToString(), "</td>" };
                            str = string.Concat(str4);
                            str = string.Concat(str, "<td style='border:1px solid gray;text-align: center'>", transferRcvInfo.Rows[i]["price"].ToString(), "</td>");
                            str = string.Concat(str, "<td style='border:1px solid gray'>", transferRcvInfo.Rows[i]["remarks"].ToString(), "</td>");
                            str = string.Concat(str, "</tr>");
