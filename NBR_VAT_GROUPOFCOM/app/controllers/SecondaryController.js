...
var SecondaryController = function ($scope, $filter, $log, $window, simsService) {
  ...
  $scope.popitupLoadSheetSale = function (Entity) {


        Entity.start_date = $("#start_date").val();
        Entity.end_date = $("#end_date").val();
        return $window.open(baseReportUrl + "LoadSheetReportSale?start_date=" + Entity.start_date + "&end_date=" + Entity.end_date + "&asr_id=" + Entity.asr_id, '_blank', 'height=800,width=1000');
    };
    ...
    $scope.getIndentByCompanyWiseSales = function (Entity) {
        ...
        response.then(function (list) {
            ...
            for (var i = 0; i < $scope.IndentObj.indentDetailsList.length; i++) {
                var pcsqty = parseInt($scope.IndentObj.indentDetailsList[i].quantity_indent);
    ...
    $scope.getIndentByCompanyWise = function (Entity) {
        ...
        response.then(function (list) {
            ...
            for (var i = 0; i < $scope.IndentObj.indentDetailsList.length; i++) {
                ...
                var quantitySc = parseInt($scope.IndentObj.indentDetailsList[i].quantity_indent);
