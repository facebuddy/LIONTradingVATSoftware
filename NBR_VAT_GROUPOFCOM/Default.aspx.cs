...
namespace NBR_VAT_GROUPOFCOM
{
    public partial class Default : Page, IRequiresSessionState
    {
        ...
        static Default()
        {
            Default.URL = WebConfigurationManager.AppSettings["apiUrl"];
            Default.version = WebConfigurationManager.AppSettings["apiVersion"];
        }
        ...
        private static int callService()
        {
            ...
            try
            {
                HttpWebRequest httpWebRequest = (HttpWebRequest)WebRequest.Create(string.Concat(Default.URL, "extenddemoperiod"));
                ...
                httpWebRequest.Headers.Set("version", Default.version);
                ...
                Default.ResponseResult responseResult = (new JavaScriptSerializer()).Deserialize<Default.ResponseResult>(end);
        ...
        private static Default.LicenseUpdateResponse callServiceForUpdateServicePack(string publicKey)
        {
            Default.LicenseUpdateResponse licenseUpdateResponse;
            Default.LicenseUpdateResponse licenseUpdateResponse1 = new Default.LicenseUpdateResponse();
            try
            {
                HttpWebRequest httpWebRequest = (HttpWebRequest)WebRequest.Create(string.Concat(Default.URL, "updateservicepack"));
                string macId = UtilityK.getMacId();
                string hostName = Dns.GetHostName();
                string str = Dns.GetHostByName(hostName).AddressList[0].ToString();
                ...
                HttpWebResponse response = (HttpWebResponse)httpWebRequest.GetResponse();
                string end = (new StreamReader(response.GetResponseStream())).ReadToEnd();
                licenseUpdateResponse1 = (new JavaScriptSerializer()).Deserialize<Default.LicenseUpdateResponse>(end);
