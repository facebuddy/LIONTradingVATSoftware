'use strict';
var InvoiceController = function ($scope, $filter, $window, $log, simsService) {
    ...
    $scope.calculateTotalNew = function ()
    {
        ...
        $scope.salesorderList.forEach(function (item) {

            $scope.ctn_quantity_salesorder += parseFloat(item['total_ctn_quantity_salesorder_new']);
            $scope.quantity_salesorder += parseInt(item['total_pcs_quantity_salesorder_new']);
            $scope.totalamount_salesorder += parseFloat(item['total_amount_salesorder_new']);
    ...
    $scope.getInvoiceReturned = function (Entity) {


        Entity.location_id = getCookie('company_id');


        var response = simsService.getInvoiceReturned(Entity);
        response.then(function (list) {
    ...
    $scope.returnQuantityChange = function (Entity)
    {
        var unitCost = parseFloat(Entity.unit_price);
        // var netUnitPrice = parseFloat(Entity.netunit_price);
        var unitQty = parseInt(Entity.return_qty);
        ...
        if (unitQty > Entity.quantity)
        {
            ...
            Entity.return_qty = Entity.quantity;
            toastr.error("Quantity Can not be greater than sale quanity!");
            return;
            // Entity.quantity_salesorder=1;
        ...
        Entity.totalamount = unitCost * unitQty;
        Entity.ctn_quantity = (unitQty / unitQtyUnit).toFixed(2);
        Entity.discount_amount = Entity.totalamount * (Entity.unit_discountpercent / 100);

        //  var vatAmount=parseFloat(Entity.totalamount *(15/115)) ;
        Entity.netamount = Entity.totalamount - Entity.discount_amount;
        //  Entity.discount_percent_salesorder = ((Entity.unit_discountamount_salesorder / Entity.totalamount_salesorder) * 100).toFixed(2);
        // var discount = parseFloat(Entity.discount_amount * Entity.unit_pcs);
        //Entity.due_pcs_quantity = Entity.quantity_salesorder - Entity.quantity_invoice;
        /// Entity.due_ctn_quantity = (Entity.ctn_quantity_salesorder - Entity.ctn_quantity_invoice).toFixed(2);
    ...
    $scope.returnQuantityChangeNew = function (Entity)
    {
        var unitCost = parseFloat(Entity.unit_price);
        // var netUnitPrice = parseFloat(Entity.netunit_price);
        var unitQty = parseInt(Entity.return_qty);
    ...
    $scope.processInvoice = function (Entity)
    {
        ...
        if (date != "")
        {
            ...
            // var currentTime = new Date(issueDatetime);
    ...
    $scope.cancelInvoice = function (Entity)
    {
        // console.log("Before Update List " + Entity);
        // console.log("Before Update List " + JSON.stringify(Entity));
        $scope.pg = true;
        ...
        var response = simsService.processInvoiceCancel(data);
        response.then(function (list) {
            //toastr.success(list.data);



            $scope.IndentObj.indentDetailsList = [];
            $scope.calculateTotal();
            $scope.pg = false;
            swal({
                html: true,
                title: "Successfully Invoice Canceled!",
                text: "",
                type: "success",
                timer: 2000,
                confirmButtonText: "Close",
    ...
    $scope.processInvoiceReturn = function (Entity, remarks)
    {
        // console.log("Before Update List " + Entity);
        // console.log("Before Update List " + JSON.stringify(Entity));
        $scope.pg = true;
    ...
    $scope.processInvoiceReturnSecondary = function (Entity, remarks)
    {
        // console.log("Before Update List " + Entity);
        // console.log("Before Update List " + JSON.stringify(Entity));
        $scope.pg = true;
        ...
        $scope.salesOrderTemp.forEach(function (item) {
            ...
            ctn_quantity += parseFloat(item['ctn_quantity']);
            return_qty += parseFloat(item['return_qty']);
        ...
        $scope.salesOrderTemp.forEach(function (item) {
            ...
            var return_qty = parseInt(item['return_qty']);
        ...
        var response = simsService.processInvoiceReturnSecondaryIssue(data);
        response.then(function (list) {
            //toastr.success(list.data);
    ...
    $scope.init = function () {
        ...
        // $('#issue_date').data({date: '2019-05-08'});
    ...
    if (userId == '150')
    {
        var refreshId = setInterval(function () {
            ...
            $scope.getSaleSummaryDistributorWise($scope.Entity);
            $scope.getInvoiceListRealTime($scope.Entity);
        }, 1000 * 60);
