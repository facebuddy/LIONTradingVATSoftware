'use strict';
var SecondaryDeliveryController = function ($scope, $filter, $log, $window, simsService) {
    ...
    $scope.DisplayModal = function (mode, title, id) {
        ...
        } else if (mode == 'DisplayNew') {
            $scope.Add = false;
            $scope.Update = false;
            $scope.title = title;
            $scope.Display = true;
            var Entity = {};
            $scope.getIndentByIdNew(id);
            $scope.btnRecieve = true;
            //  $scope.calculateTotal();
    ...
    $scope.processSalesOrder = function (Entity, IndentObj)
    {
        ...
        if (typeof $scope.IndentObj.delivery_person_id == "undefined" || typeof $scope.IndentObj.delivery_person_id == null)
        {
        ...
        $scope.salesOrderTemp.forEach(function (item) {
            ...
            var qty = parseInt(item['quantity_salesorder']);

            // qty = 2000;
            if (isNaN(qty))
            {
                toastr.error(item['product_name'] + "Quantiy can not be zero or empty! " + qty + " ");
                avlFlg = false;
                return;
            }
        ...
        // return;
        var groupResult = [];
        groupResult = $scope.salesOrderTemp;
        var result = [];
        groupResult.reduce(function (res, value) {
            if (!res[value.chalan_master_id]) {
                res[value.chalan_master_id] = {Id: value.chalan_master_id, qty: 0, discount_amount_salesorder: 0.00, netamount_salesorder: 0.00, netamount_salesorder: 0.00, totalamount_salesorder: 0.00};
                result.push(res[value.chalan_master_id])
            }
            res[value.chalan_master_id].qty += value.quantity_salesorder;
            res[value.chalan_master_id].discount_amount_salesorder += parseFloat(value.discount_amount_salesorder);
            res[value.chalan_master_id].netamount_salesorder += parseFloat(value.netamount_salesorder);
            res[value.chalan_master_id].totalamount_salesorder += parseFloat(value.totalamount_salesorder);
            return res;
        }, {});
        var orderSummary = [];
        orderSummary = result;
        ...
        groupResult.reduce(function (res, value) {
            if (!res[value.product_id]) {
                res[value.product_id] = {Id: value.product_id, product_name: "", qty: 0, stock: 0};
                result.push(res[value.product_id])
            }
            res[value.product_id].qty += parseInt(value.quantity_salesorder);
            res[value.product_id].netamount_salesorder += parseInt(value.netamount_salesorder);
            res[value.product_id].stock = parseInt(value.available_stock);
        ...
        orderDetails.forEach(function (item) {
            ...
            var stockAvail = parseInt(item['available_stock']);
            // qty = 2000;
            if (qty > stock)
            {
                toastr.error("Stock Net Available! " + item['product_name'] + " Total Delivery Quantiy: " + qty + " Total Stock: " + stock);
                avlFlag = false;
                return;
            }
        ...
        if (avlFlag == false || avlFlg == false)
        {
            $scope.pg = false;
            // toastr.error("Stock Net Available!");
            return;
        }
        ...
        var response = simsService.processSalesOrderSecondaryNew(data);
    ...
    $scope.processInvoiceSecondaryCancel = function (Entity, IndentObj)
    {
        ...
        var groupResult = $scope.salesOrderTemp;
        ...
        groupResult.reduce(function (res, value) {
            if (!res[value.chalan_master_id]) {
                res[value.chalan_master_id] = {Id: value.chalan_master_id, company_id: 0, outlet_id: 0, user_id: 0, bit_id: 0, asr_id: 0, company_id: 0, qty: 0, discount_amount_salesorder: 0.00, netamount_salesorder: 0.00, netamount_salesorder: 0.00, totalamount_salesorder: 0.00};
                result.push(res[value.chalan_master_id])
            }
            res[value.chalan_master_id].qty += value.sale_quantity;
            res[value.chalan_master_id].outlet_id = value.outlet_id;
            res[value.chalan_master_id].asr_id = value.asr_id;
            res[value.chalan_master_id].bit_id = value.bit_id;
            res[value.chalan_master_id].user_id = getCookie('user_id');
            res[value.chalan_master_id].company_id = getCookie('company_id');
            res[value.chalan_master_id].discount_amount_salesorder += parseFloat(value.discount_amount_salesorder);
            res[value.chalan_master_id].netamount_salesorder += parseFloat(value.netamount_salesorder);
            res[value.chalan_master_id].totalamount_salesorder += parseFloat(value.totalamount_salesorder);
            return res;
        }, {});
        var orderSummary = result;
        orderSummary.forEach(function (items) {


            items.salesDetail = $filter('filter')($scope.salesOrderTemp, function (item) {
                return (item.chalan_master_id == items.Id);
            });
        });
        ...
        // console.log("Process sales order data " + JSON.stringify(orderSummary));
        //$scope.pg = false;
        //         return;


        // console.log("Process sales order data " + JSON.stringify(data));
        var response = simsService.processInvoiceSecondaryCancel(orderSummary);
        response.then(function (list) {
            //toastr.success(list.data);
            console.log("success" + JSON.stringify(list.data));
            $scope.processBtn = false;
            // $("#processBtn").attr("disabled", true);
            $scope.pg = false;
            swal({
                html: true,
                title: JSON.stringify(list.data),
                text: "",
                type: "success",
                timer: 2000,
                confirmButtonText: "Close",
    ...
    $scope.processDelivery = function (Entity, IndentObj)
    {
        ...
        groupResult.reduce(function (res, value) {
            if (!res[value.chalan_master_id]) {
                res[value.chalan_master_id] = {Id: value.chalan_master_id, company_id: 0, outlet_id: 0, user_id: 0, bit_id: 0, asr_id: 0, company_id: 0, qty: 0, discount_amount_salesorder: 0.00, netamount_salesorder: 0.00, netamount_salesorder: 0.00, totalamount_salesorder: 0.00,return_quantity_salesorder:0};
                result.push(res[value.chalan_master_id])
            }
            res[value.chalan_master_id].qty += value.sale_quantity;
            res[value.chalan_master_id].outlet_id = value.outlet_id;
            ...
            res[value.chalan_master_id].netamount_salesorder += parseFloat(value.netamount_salesorder);
            res[value.chalan_master_id].totalamount_salesorder += parseFloat(value.totalamount_salesorder);
            res[value.chalan_master_id].return_quantity_salesorder += parseInt(value.return_quantity_salesorder);
            return res;
        }, {});
        var orderSummary = result;
        orderSummary.forEach(function (items) {
        ...
        console.log("Detail" + JSON.stringify(orderSummary));
        var groupResult = $scope.salesOrderTemp;
        var result = [];
        groupResult.reduce(function (res, value) {
            if (!res[value.product_id]) {
            ...
            res[value.product_id].qty += parseInt(value.quantity_salesorder);
            res[value.product_id].stock = parseInt(value.stock);
            res[value.product_id].return_quantity_salesorder += parseInt(value.return_quantity_salesorder);
            res[value.product_id].product_name = value.product_name;
            return res;
        }, {});
        var orderDetails = result;
      ...
      //  $scope.pg = false;
        ...
        // console.log("Process sales order data " + JSON.stringify(data));
        var response = simsService.processInvoiceSecondaryNew(orderSummary);
        response.then(function (list) {
            //toastr.success(list.data);
            console.log("success" + JSON.stringify(list.data));
            $scope.processBtn = false;
    ...
    $scope.UpdateModal = function (mode, title, id) {
        if (mode == 'Add') {
            ...
            $("#trackID").val(id);
            // trackId
            $scope.getBitListByCompany($scope.Entity.distributor_company_id);
            $scope.Entity = $filter('filter')($scope.outletList, function (d) {
                return d.id === id;
            })[0];
            if ($scope.Entity.division) {
                $scope.getDistricts($scope.Entity.division);
                $scope.Entity.division = $scope.Entity.division;
                $scope.Entity.district = $scope.Entity.district;
                //do something
            }

            if ($scope.Entity.upozella) {
                $scope.getUpazila($scope.Entity.division);
                $scope.Entity.division = $scope.Entity.division;
                $scope.Entity.upozella = $scope.Entity.upozella;
                // getUpazila
                //do something
            }
    ...
    $scope.saveChallanReturn = function (Entity) {
    ...
    $scope.getDeliveryPersonList = function (Entity) {
        ...
        var data = {
            location_id: getCookie('company_id'),
            user_type: Entity
        };
    ...
    $scope.popitupLoadSheet = function (Entity) {


        Entity.start_date = $("#start_date").val();
        Entity.end_date = $("#end_date").val();
        return $window.open(baseReportUrl + "LoadSheetReport?start_date=" + Entity.start_date + "&end_date=" + Entity.end_date + "&asr_id=" + Entity.asr_id, '_blank', 'height=800,width=1000');
    };
    $scope.popitupReport = function (url, type) {

        if (type == 1)
        {
            return $window.open(baseReportUrl + "StockReceiveDetails?invoice_no=" + url + "&type=" + type, '_blank', 'height=800,width=1000');
        }
    ...
    $scope.getDeliveryListSecondary = function (Entity) {
        ...
        Entity.end_date = $("#end_date").val();
        Entity.company_id = getCookie('company_id');
        var response = simsService.getDeliveryListSecondaryNew(Entity);
        response.then(function (list) {
    ...
    $scope.getChalanReport = function (Entity) {

        $scope.pg = true;
        Entity.start_date = $("#start_date").val();
        Entity.end_date = $("#end_date").val();
        Entity.location_id = getCookie('company_id');
        var response = simsService.getchalanReportSecondary(Entity);
        response.then(function (list) {

            $scope.chalanList = list.data;
            $scope.calculateTotalChallan();
            $scope.pg = false;
            console.log("Indent List" + JSON.stringify($scope.chalanList));
        }, function (error) {
            toastr.error(error.data);
        });
    }
    ...
    $scope.CheckUncheckHeaderProcess = function () {
        // $scope.IsAllChecked = true;
    ...
    $scope.stockReceive = function (stockList, Entity)
    {
        ...
        var data = {
            ...
            invoice_no: Entity[0].invoice_no,
            stockList: stockList,
        };
    ...
    $scope.stockReceivePrimary = function (stockList, Entity)
    {
        // console.log("Before Update List " + Entity);
        // console.log("Before Update List " + JSON.stringify(Entity));
        $scope.pg = true;
        var data = {
            company_id: 1,
            user_id: getCookie('user_id'),
            total_amount: Entity[0].total_amount,
            tracking_no: Entity[0].indent_number,
            net_amount: Entity[0].net_amount,
            total_qty_pcs: Entity[0].total_qty_pcs,
            total_qty_ctn: Entity[0].total_qty_ctn,
            distribution_node_id: Entity[0].origin_location_id,
            ref_invoice_no: Entity[0].ref_invoice_no,
            salesmaster_id: Entity[0].salesmaster_id,
            stockList: stockList,
        };
        var response = simsService.stockReceivePrimary(data);
        response.then(function (list) {
            //toastr.success(list.data);
    ...
    $scope.getUpazila = function (Entity) {


        var response = simsService.getUpazila(Entity);
        response.then(function (list) {

            $scope.upazilaList = list.data;
            $scope.pg = false;
            console.log("Bit list" + JSON.stringify($scope.upazilaList));
            // toastr.success(list.data);
    ...
    $scope.getReturnInvoiceListPrimary = function (Entity) {
        ...
        Entity.end_date = $("#end_date").val();
        Entity.location_id = getCookie('company_id');
        var response = simsService.getReturnInvioceListByLocationPrimary(Entity);
        response.then(function (list) {
    ...
    $scope.calculateTotalSalesOrder = function ()
    {
        ...
        $scope.total_netamount = 0;
        $scope.IndentObj.indentDetailsList.forEach(function (item) {
    ...
    $scope.calculateTotalChallan = function ()
    {
        $scope.total_qty = 0;
        $scope.total_netamount = 0;
        $scope.chalanList.forEach(function (item) {

            $scope.total_netamount += parseFloat(item['total_netamount']);
            $scope.total_qty += parseFloat(item['total_qty']);
        });
    };
    $scope.calculateTotalNew = function ()
    {
        ...
        $scope.IndentObj[0].invoiceDetailsList.forEach(function (item) {
            ...
            $scope.total_netamount += parseFloat(item['netamount_indent'])
            $scope.totalQty += parseFloat(item['quantity_indent'])
    ...
    $scope.getIndentByIdNew = function (Entity) {

        var data = {
            invoice_no: Entity
        };
        var response = simsService.getInvioceByIdNew(data);
        response.then(function (list) {
            //  $scope.IndentObj.chalanmaster_id=Entity;

            $scope.IndentObj = list.data;
            console.log("Final Result" + $scope.IndentObj);
            $scope.IsAllChecked = true;
    ...
    {
        /* var unitQtySls = parseInt(Entity.quantity_salesorder)
         if(isNaN(unitQtySls))
         {
        ...
        if (Entity.available_stock < Entity.quantity_salesorder)
        {
       ...
       // Entity.discount_amount_salesorder_unit=Entity.discount_amount_salesorder;
        Entity.discount_amount_salesorder =  Entity.discount_amount_salesorder_unit*unitQty;
        //parseFloat(Entity.totalamount_salesorder * (Entity.discount_percent_salesorder / 100).toFixed(2));
    ...
    $scope.getIndentByCompanyWiseSales = function (Entity) {
        ...
        $scope.IndentObj = {};
        var response = simsService.getsalesListSecondaryNew(data);
        response.then(function (list) {
            //  $scope.IndentObj.chalanmaster_id=Entity;
            ...
            $scope.IsAllChecked = true;
            for (var i = 0; i < $scope.IndentObj.indentDetailsList.length; i++) {
                var pcsqty = Math.abs(parseInt($scope.IndentObj.indentDetailsList[i].quantity_salesorder));
                var pcsstock = parseInt($scope.IndentObj.indentDetailsList[i].stock);
                if (pcsqty > pcsstock)
                {
                ...
                $scope.IndentObj.indentDetailsList[i].quantity_salesorder =Math.abs(parseInt($scope.IndentObj.indentDetailsList[i].quantity_salesorder));
                ...
                $scope.IndentObj.indentDetailsList[i].discount_amount_salesorder_unit = parseFloat($scope.IndentObj.indentDetailsList[i].unit_discount_amount);
    ...
    $scope.getIndentByCompanyWise = function (Entity) {
        ...
        var response = simsService.getIndentByIdSecondaryBatch(data);
        response.then(function (list) {
            //  $scope.IndentObj.chalanmaster_id=Entity;
            ...
            for (var i = 0; i < $scope.IndentObj.indentDetailsList.length; i++) {
                ...
                var quantitySc =Math.abs(parseInt($scope.IndentObj.indentDetailsList[i].quantity_indent));
                // var stockSc = parseInt($scope.IndentObj.indentDetailsList[i].stock);
                var stockSc = parseInt($scope.IndentObj.indentDetailsList[i].available_stock);

                if (quantitySc > stockSc)
                {
                    $scope.IndentObj.indentDetailsList[i].stockAvail = false;
                    // $scope.IndentObj.indentDetailsList[i].isSelected = false;
                } else
                ...
                $scope.IndentObj.indentDetailsList[i].isSelected = true;
                $scope.IndentObj.indentDetailsList[i].isNewItem = false;
                $scope.IndentObj.indentDetailsList[i].quantity_indent=Math.abs($scope.IndentObj.indentDetailsList[i].quantity_indent);

                $scope.IndentObj.indentDetailsList[i].ctn_quantity_salesorder =Math.abs($scope.IndentObj.indentDetailsList[i].ctn_quantity_indent);
                $scope.IndentObj.indentDetailsList[i].quantity_salesorder =Math.abs(parseInt($scope.IndentObj.indentDetailsList[i].quantity_indent));
                $scope.IndentObj.indentDetailsList[i].totalamount_salesorder = $scope.IndentObj.indentDetailsList[i].totalamount_indent;
    ...
    $scope.CheckUncheckHeaderNew = function () {
        // $scope.IsAllChecked = true;
    ...
    $scope.init = function () {
        ...
        // var baseUrl = 'http://127.0.0.1:8000/api/findProductQuerys';
        // var reportUrl = 'http://localhost:51592/';
        var baseUrl = 'http://sims.timezoneservice.com/api/findProductQuerys';
        var reportUrl = 'http://sims.timezoneservice.com/';
            ...
            var findProductQuery = new Bloodhound({
                ...
                remote: {
                    ...
                    replace: function (url, uriEncodedQuery) {
                        var val = getCookie('company_id');
                        if (!val)
                            return url.replace("%QUERY", uriEncodedQuery);
                        var urlNew = url.replace("%QUERY", uriEncodedQuery) + '^' + encodeURIComponent(val);
                        return url.replace("%QUERY", uriEncodedQuery) + '^' + encodeURIComponent(val)
                    },
                    wildcard: '%QUERY'
                }
            });
...
angular.module('myApps')
        .controller('SecondaryDeliveryController', SecondaryDeliveryController);